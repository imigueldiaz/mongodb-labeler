	...
	return jest.fn(() => ({
		...
		inject: jest.fn().mockImplementation(async (opts) => {
			const handlers = {
				"/xrpc/com.atproto.label.queryLabels": {
					GET: async () => {
						if (global.mockLabels) {
							return {
								statusCode: 200,
								payload: JSON.stringify({ cursor: "1", labels: global.mockLabels }),
							};
						}
						return {
							statusCode: 200,
				...
				"/xrpc/unknown.method": {
					GET: async () => ({ statusCode: 501, payload: "Method Not Implemented" }),
				},
			};
			...
			if (opts.url === "/xrpc/com.atproto.label.queryLabels" && opts.method === "GET") {
				...
				if (global.mockFindLabelsError) {
					return { statusCode: 500, payload: "Internal Server Error" };
				}
			}
			...
			return { statusCode: 404, payload: "Not Found" };
		}),
	}));
});
...
jest.mock("@fastify/websocket", () => {
	class MockSocketStream {
		constructor() {
			this.socket = { send: jest.fn(), close: jest.fn(), terminate: jest.fn() };
		}
	}

	return { __esModule: true, default: jest.fn(), SocketStream: MockSocketStream };
});

// Mock crypto utils
jest.mock(
	"./src/util/crypto",
	() => ({ parsePrivateKey: () => ({ sign: async () => new Uint8Array(64) }) }),
);
