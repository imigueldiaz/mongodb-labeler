	...
	return jest.fn(() => ({
		...
		inject: jest.fn().mockImplementation((opts: { url: string; method: string }) => {
			const routes = {
				...
				"/xrpc/unknown.method": {
					GET: () => ({ statusCode: 501, payload: "Method Not Implemented" }),
				},
			};
			...
			return Promise.resolve({ statusCode: 404, payload: "Not Found" });
		}),
		app: {},
	}));
});
...
class MockSocketStream {
	socket: { send: jest.Mock; close: jest.Mock; terminate: jest.Mock };
...
export default { __esModule: true, default: jest.fn(), SocketStream: MockSocketStream };
